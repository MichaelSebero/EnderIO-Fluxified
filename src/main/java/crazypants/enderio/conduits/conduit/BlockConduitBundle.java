    ...
    if (!YetaUtil.isSolidFacadeRendered(con, player)) {
      ...
      if (component != null) {
        ...
        if (component.isDirectional() && component.conduitType == null) {
          ...
          dir = dir.getOpposite();
          float trans = 0.0125f;
          minBB = minBB.translate(dir.getXOffset() * trans, dir.getYOffset() * trans, dir.getZOffset() * trans);
          float scale = 0.7f;
          minBB = minBB.scale(1 + Math.abs(dir.getXOffset()) * scale, 1 + Math.abs(dir.getYOffset()) * scale,
              1 + Math.abs(dir.getZOffset()) * scale);
        } else {
