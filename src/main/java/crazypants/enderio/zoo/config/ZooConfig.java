...
public final class ZooConfig {
    ...
    public static final IValue<Float> lootModifierFakePlayer = F.make("lootModifierFakePlayer", .5f, //
            "Adjust the probability of 'player kill' loot drops for fake players (machines). Does not affect normal drops or XP.")
            .setRange(0, 1).sync();
    ...
    public static final IValue<Integer> confusionDuration = CREEPER.make("confusionDuration", 100, //
            "Sets the durtaion in ticks of the confusion effect applied on explosion.").setRange(20 * 1, 20 * 30)
            .sync();
    ...
    public static final IValue<Float> direSlime2Chance = SLIME.make("direSlime2Chance", .4f, //
            "The chance that a medium Dire Slime will spawn when a small Dire Slime is killed (eg 0.12 for a 12% chance).")
            .setRange(0, 1).sync();
    public static final IValue<Float> direSlime3Chance = SLIME.make("direSlime3Chance", .2f, //
            "The chance that a large Dire Slime will spawn when a medium Dire Slime is killed (eg 0.12 for a 12% chance).")
            .setRange(0, 1).sync();
    ...
    public static final IValue<Float> howlPackChance = WOLF.make("howlPackChance", .6f, //
            "The chance that when a dire wolf howls, nearby dire wolves will \"join in\" to a pack howl.")
            .setRange(0, 1);
    ...
    public static final IValue<Boolean> fallKnightMountedArchersMaintainDistance = KNIGHT.make(
            "mountedArchersMaintainDistance", true, //
            "When true mounted archer knights will attempt to keep distance between themselves and their target.")
            .sync();
    ...
    public static final IValue<Float> fallenKnightChanceArmorUpgradeHard = KNIGHT.make("chanceArmorUpgradeHard", .4f, //
            "The chance the type of armor equipped will be improved when difficulty is set to hard.").setRange(0, 1)
            .sync();
