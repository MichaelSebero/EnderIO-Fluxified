...
public class EntityVoidSlime extends EntityMagmaCube implements IEnderZooEntity.Aggressive {
    ...
    private static final @Nonnull EntityLiving.SpawnPlacementType ON_BEDROCK = NullHelper.first(
            EnumHelper.addSpawnPlacementType("ON_BEDROCK",
                    (world, pos) -> pos != null && pos.getY() < 32 &&
    ...
    public void setSlimeSize(int size, boolean doFullHeal) {
        ...
        getAttributeMap().getAttributeInstance(SharedMonsterAttributes.ATTACK_DAMAGE)
                .setBaseValue(ZooConfig.voidSlimeAttackDamage.get());
    ...
    private void onLivingUpdateClient() {
        if (actionDelay-- <= 0) {
            ...
            if (particles.size() < 10) {
                ...
                float maxSize = rand.nextFloat() *
                        (rand.nextBoolean() ? (2 * ZooConfig.voidSlimeRange.get() + 0.1f) : 3.9f);
                float color = rand.nextBoolean() ? 0 : rand.nextFloat() / 10;
                final InfinityParticle particle = new InfinityParticle(world, BlockCoord.get(this),
                        new Vector4f(color, color, color, 1f),
    ...
    /**
     * Get a list of all players in range. Derived from
     * {@link World#getClosestPlayer(double, double, double, double, Predicate)}.
     */
    ...
    private class BlindEffect extends PotionEffect {
        ...
        public boolean onUpdate(@Nonnull EntityLivingBase entityplayer) {
            if (combined || (!dead && entityplayer.getDistanceSq(posX, posY, posZ) <
                    ZooConfig.voidSlimeRange.get() * ZooConfig.voidSlimeRange.get())) {
        ...
        public void combine(@Nonnull PotionEffect other) {
            if (!(other instanceof BlindEffect)) {
                // we got combined with a normal blindness effect. this means we should no longer vanish when the player
                // gets out of range.
                combined = true;
            }
            super.combine(other);
