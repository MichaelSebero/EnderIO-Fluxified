...
public interface IEnderZooEntity extends IAnimals {
    ...
    static void register(@Nonnull Register<EntityEntry> event, @Nonnull String name,
                         @Nonnull Class<? extends Entity> clazz, int eggBgCol, int eggFgCol,
                         IMobID id) {
        ...
        EntityRegistry.registerModEntity(entry.getRegistryName(), entry.getEntityClass(), entry.getName(), id.getID(),
                EnderIOZoo.MODID, 64, 3, true);
    }

    default void applyAttributes(@Nonnull EntityLivingBase entity, @Nonnull IValue<Double> baseHealth,
                                 @Nonnull IValue<Double> baseAttack) {
        entity.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.MAX_HEALTH)
                .setBaseValue(baseHealth.get());
