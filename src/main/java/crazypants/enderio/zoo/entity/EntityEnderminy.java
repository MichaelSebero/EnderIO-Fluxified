...
public class EntityEnderminy extends EntityMob implements IEnderZooEntity {
    ...
    private static final @Nonnull DataParameter<Boolean> SCREAMING_INDEX = EntityDataManager
            .<Boolean>createKey(EntityEnderminy.class, DataSerializers.BOOLEAN);

    private static final @Nonnull UUID attackingSpeedBoostModifierUUID = UUID
            .fromString("020E0DFB-87AE-4653-9556-831010E291B0");
    private static final @Nonnull AttributeModifier attackingSpeedBoostModifier = (new AttributeModifier(
    ...
    public EntityEnderminy(World world) {
        ...
        targetTasks.addTask(2,
                new EntityAINearestAttackableTarget<EntityCreeper>(this, EntityCreeper.class, true, true) {
    ...
    public boolean shouldAttackPlayer(EntityPlayer player) {
        ...
        if (itemstack.getItem() == Item.getItemFromBlock(Blocks.PUMPKIN)) {
            ...
            Vec3d relativePlayerEyePos = new Vec3d(posX - player.posX,
                    getEntityBoundingBox().minY + height / 2.0F - (player.posY + player.getEyeHeight()),
                    posZ - player.posZ);

            double distance = relativePlayerEyePos.length();
    ...
    public boolean teleportToEntity(Entity p_70816_1_) {
        Vec3d vec3 = new Vec3d(posX - p_70816_1_.posX,
    ...
    public boolean attackEntityFrom(@Nonnull DamageSource damageSource, float p_70097_2_) {
        ...
        if (damageSource instanceof EntityDamageSource && damageSource.getTrueSource() instanceof EntityPlayer &&
                getHealth() > 0) {
            boolean skipTeleport = SwordHandler.isEquippedAndPowered((EntityPlayer) damageSource.getTrueSource(),
                    DarkSteelConfig.darkSteelSwordPowerUsePerHit);
            isAggressive = true;
            if (rand.nextInt(3) == 0) {
                for (int i = 0; i < 64; ++i) {
    ...
    private void doGroupArgo() {
        ...
        int range = 16;
        AxisAlignedBB bb = new AxisAlignedBB(posX - range, posY - range, posZ - range, posX + range, posY + range,
                posZ + range);
