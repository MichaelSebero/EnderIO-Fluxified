...
public class EntityWitherCat extends EntityMob
                             implements IOwnable<EntityWitherCat, EntityWitherWitch>, IEnderZooEntity {
    ...
    private static final @Nonnull DataParameter<Float> SCALE_INDEX = EntityDataManager
            .<Float>createKey(EntityWitherCat.class, DataSerializers.FLOAT);
    private static final @Nonnull DataParameter<Integer> GROWTH_MODE_INDEX = EntityDataManager
            .<Integer>createKey(EntityWitherCat.class, DataSerializers.VARINT);
    ...
    protected void updateAttackDamage(float growthRatio) {
        IAttributeInstance att = EntityUtil.removeModifier(this, SharedMonsterAttributes.ATTACK_DAMAGE,
                ATTACK_BOOST_MOD_UID);
    ...
    protected void updateHealth(float growthRatio) {
        IAttributeInstance att = EntityUtil.removeModifier(this, SharedMonsterAttributes.MAX_HEALTH,
                HEALTH_BOOST_MOD_UID);
        if (growthRatio == 0 || att == null) {
            return;
        }
    ...
    private void spawnParticles() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            Particle fx = Minecraft.getMinecraft().effectRenderer.spawnEffectParticle(
                    EnumParticleTypes.SPELL.getParticleID(), startX + xOffset, startY + yOffset,
