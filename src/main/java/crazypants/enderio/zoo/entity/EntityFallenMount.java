...
public class EntityFallenMount extends EntityHorse implements IEnderZooEntity.Aggressive {
    ...
    public IEntityLivingData onInitialSpawn(@Nonnull DifficultyInstance di, @Nullable IEntityLivingData data) {
        ...
        float chanceOfArmor = (world.getDifficulty() == EnumDifficulty.HARD ? ZooConfig.fallenMountChanceArmoredHard :
                ZooConfig.fallenMountChanceArmored).get();
        if (rand.nextFloat() <= chanceOfArmor) {
            ...
            float chanceImprovedArmor = (world.getDifficulty() == EnumDifficulty.HARD ?
                    ZooConfig.fallenMountChanceArmorUpgradeHard : ZooConfig.fallenMountChanceArmorUpgrade).get();
    ...
    public void onLivingUpdate() {
        ...
        if (world.isDaytime() && !world.isRemote) {
            if (burnInSun() && world.getTotalWorldTime() % 20 == 0) {
                ...
                if (f > 0.5F && rand.nextFloat() * 30.0F < (f - 0.4F) * 2.0F && world.canBlockSeeSky(
