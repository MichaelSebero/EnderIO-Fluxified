...
public class EntityWitherWitch extends EntityMob implements IRangedAttackMob, IEnderZooEntity.Aggressive {
    ...
    protected void doAttackActions() {
        ...
        // the EntityPotion class validates if this potion is throwable, and if not it logs error "ThrownPotion entity
        // {} has no item?!
    ...
    private void spawnCats() {
        ...
        for (int i = 0; i < numCats; i++) {
            BlockPos clearGround = SpawnUtil.findClearGround(world,
                    BlockCoord.get(this).add(4 - rand.nextInt(9), 0, 4 - rand.nextInt(9)), 2, 10, true);
            if (clearGround != null) {
    ...
    private void spawnCat(BlockPos spawnLoc) {
        ...
        if (MinecraftForge.EVENT_BUS.post(new LivingSpawnEvent.CheckSpawn(cat, world, (float) cat.posX,
                (float) cat.posY, (float) cat.posZ, false))) {
            return;
        }
