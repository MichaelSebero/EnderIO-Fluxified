...
public class EntityFallenKnight extends EntitySkeleton implements IEnderZooEntity.Aggressive {
    ...
    private final @Nonnull EntityAIMountedArrowAttack aiArrowAttack = new EntityAIMountedArrowAttack(this,
            ZooConfig.fallenKnightChargeSpeed,
            ZooConfig.fallenMountChargeSpeed, ZooConfig.fallenKnightRangedMinAttackPause,
            ZooConfig.fallenKnightRangedMaxAttackPause,
            ZooConfig.fallenKnightRangedMaxRange, ZooConfig.fallKnightMountedArchersMaintainDistance);
    private final @Nonnull EntityAIMountedAttackOnCollide aiAttackOnCollide = new EntityAIMountedAttackOnCollide(this,
            EntityPlayer.class,
    ...
    public EntityFallenKnight(World world) {
        ...
        targetTasks.addTask(2,
                new EntityAINearestAttackableTarget<AbstractIllager>(this, AbstractIllager.class, false));
    }
    ...
    public void onLivingUpdate() {
        ...
        if (ZooConfig.fallenKnightArchersSwitchToMelee.get() &&
                (!isMounted || !ZooConfig.fallKnightMountedArchersMaintainDistance.get()) && attackTarget != null &&
    ...
    private void addRandomArmor() {
        ...
        float chancePerPiece = (isHardDifficulty() ? ZooConfig.fallenKnightChancePerArmorPiece :
                ZooConfig.fallenKnightChancePerArmorPieceHard).get();
    ...
    private int getRandomEquipmentLevel(float occupiedDiffcultyMultiplier) {
        float chanceImprovedArmor = (isHardDifficulty() ? ZooConfig.fallenKnightChanceArmorUpgradeHard :
                ZooConfig.fallenKnightChanceArmorUpgrade).get();
    ...
    private @Nonnull ItemStack getSwordForLevel(int swordLevel) {
        ...
        if (rand.nextFloat() < .80f) {
            switch (swordLevel) {
                ...
                default:
                    return rand.nextBoolean() ? new ItemStack(Items.IRON_SWORD) :
                            new ItemStack(ModObject.itemDarkSteelSword.getItemNN());
            }
        } else {
            switch (swordLevel) {
                case 0:
                ...
                default:
                    return rand.nextBoolean() ? new ItemStack(Items.IRON_AXE) :
                            new ItemStack(ModObject.itemDarkSteelAxe.getItemNN());
            }
        }
    }
