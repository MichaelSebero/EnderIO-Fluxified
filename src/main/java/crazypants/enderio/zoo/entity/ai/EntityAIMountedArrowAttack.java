...
public class EntityAIMountedArrowAttack extends EntityAIBase {
    ...
    public EntityAIMountedArrowAttack(@Nonnull IRangedAttackMob host, IValue<Double> moveSpeed,
                                      IValue<Double> mountedEntityMoveSpeed,
                                      IValue<Integer> minAttackTime, IValue<Integer> maxAttackTime,
                                      IValue<Float> attackRange, IValue<Boolean> useRunAwayTactic) {
    ...
    public void updateTask() {
        ...
        double distToTargetSq = entityHost.getDistanceSq(attackTarget.posX, attackTarget2.getEntityBoundingBox().minY,
                attackTarget.posZ);
        ...
        if (--timeUntilNextAttack == 0) {
            ...
            timeUntilNextAttack = MathHelper.floor(
                    rangeRatio * (maxRangedAttackTime.get() - minRangedAttackTime.get()) + minRangedAttackTime.get());
    ...
    private boolean runAway() {
        ...
        BlockPos probePoint = new BlockPos((int) Math.round(targetDir.x), (int) Math.round(entityHost.posY),
                (int) Math.round(targetDir.z));
        ...
        boolean res = getNavigator().tryMoveToXYZ(clearGround.getX() + .5, clearGround.getY(), clearGround.getZ() + .5,
                mountedEntityMoveSpeed.get());
        if (getNavigator().noPath()) {
            runningAwayTo = null;
        } else {
            final Path path = getNavigator().getPath();
