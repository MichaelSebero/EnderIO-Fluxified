...
public class AIFindPlayer extends EntityAINearestAttackableTarget<EntityPlayer> {
    ...
    public boolean shouldExecute() {
        ...
        List<EntityPlayer> list = taskOwner.world.getEntitiesWithinAABB(EntityPlayer.class,
                taskOwner.getEntityBoundingBox().expand(d0, 4.0D, d0),
    ...
    public void updateTask() {
        ...
        if (targetPlayer2 != null) {
            if (--stareTimer <= 0) {
                ...
                IAttributeInstance iattributeinstance = enderminy
            ...
            if (NullHelper.untrust(targetEntity) != null) {
                if (enderminy.shouldAttackPlayer(this.targetEntity)) {
                    ...
                    teleportDelay = 0;
                } else if (targetEntity.getDistanceSq(enderminy) > 256.0D && this.teleportDelay++ >= 30 &&
                        enderminy.teleportToEntity(targetEntity)) {
                            teleportDelay = 0;
                        }
            }
            super.updateTask();
