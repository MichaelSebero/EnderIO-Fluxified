...
public class EntityAIFlyingFindPerch extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        BlockPos targetPos = EntityUtil.findRandomLandingSurface(entity, searchRange, entity.getPosition().getY() + 1,
                250, searchAttempts);
        if (targetPos != null) {
            List<EntityCreature> others = entity.getEntityWorld().getEntitiesWithinAABB(entity.getClass(),
                    EntityUtil.getBoundsAround(targetPos, 4));
            if (others.size() > 1) {
                return false;
            }
