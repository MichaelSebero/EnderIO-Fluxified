...
public class EntityAIMountedAttackOnCollide extends EntityAIBase {
    ...
    public EntityAIMountedAttackOnCollide(EntityCreature attacker, Class<?> targetClass,
                                          IValue<Double> speedTowardsTarget,
    ...
    public EntityAIMountedAttackOnCollide(EntityCreature attacker, IValue<Double> speedTowardsTarget,
                                          IValue<Double> speedTowardsTargetMounted,
                                          boolean longMemory) {
    ...
    public boolean shouldContinueExecuting() {
        EntityLivingBase entitylivingbase = attacker.getAttackTarget();
        return entitylivingbase == null ? false : (!entitylivingbase.isEntityAlive() ? false :
                (!longMemory ? !getNavigator().noPath() : attacker.isWithinHomeDistanceCurrentPosition()));
    }
    ...
    public void updateTask() {
        ...
        double distanceFromAttackerSq = attacker.getDistanceSq(target.posX, target.getEntityBoundingBox().minY,
                target.posZ);
        if ((longMemory || attacker.getEntitySenses().canSee(target)) && pathUpdateTimer <= 0 &&
            ...
            if (path != null) {
                ...
                if (finalPathPoint != null &&
                        target.getDistanceSq(finalPathPoint.x, finalPathPoint.y, finalPathPoint.z) < 1) {
