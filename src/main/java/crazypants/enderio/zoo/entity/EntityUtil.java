...
public class EntityUtil {
    ...
    public static float getDifficultyMultiplierForLocation(@Nonnull World world, double x, double y, double z) {
        ...
        // occupied
        float occupiedDiffcultyMultiplier = world.getDifficultyForLocation(new BlockPos(x, y, z))
    ...
    public static @Nullable IAttributeInstance removeModifier(@Nonnull EntityLivingBase ent, @Nonnull IAttribute p,
                                                              @Nonnull UUID u) {
    ...
    public static double getDistanceSqToNearestPlayer(@Nonnull Entity entity, double maxRange) {
        ...
        EntityPlayer nearest = (EntityPlayer) entity.getEntityWorld().findNearestEntityWithinAABB(EntityPlayer.class,
                bounds, entity);
        if (nearest == null) {
            return 1;
        }
    ...
    public static boolean isPlayerWithinRange(@Nonnull Entity entity, double range) {
        List<EntityPlayer> res = entity.getEntityWorld().getEntitiesWithinAABB(EntityPlayer.class,
                getBoundsAround(entity, range));
        return !res.isEmpty();
    }

    public static boolean isOnGround(@Nonnull EntityCreature entity) {
        List<AxisAlignedBB> collides = entity.getEntityWorld().getCollisionBoxes(entity,
                entity.getEntityBoundingBox().offset(0, -0.1, 0));
        if (collides.isEmpty()) {
            return false;
        }
    ...
    public static @Nullable BlockPos findRandomLandingSurface(@Nonnull EntityCreature entity, int searchRange, int minY,
                                                              int maxY, int searchAttempts) {
    ...
    public static @Nullable BlockPos findRandomClearArea(@Nonnull EntityCreature entity, int searchRange, int minY,
                                                         int maxY, int searchAttempts) {
    ...
    public static @Nullable BlockPos findRandomLandingSurface(@Nonnull EntityLiving entity, int searchRange, int minY,
                                                              int maxY) {
        BlockPos ep = entity.getPosition();
        World worldObj = entity.getEntityWorld();
        int x = ep.getX() + -searchRange + (worldObj.rand.nextInt(searchRange + 1) * 2);
    ...
    public static @Nullable BlockPos findClearLandingSurface(@Nonnull EntityLiving ent, int x, int z, int minY,
                                                             int maxY) {
