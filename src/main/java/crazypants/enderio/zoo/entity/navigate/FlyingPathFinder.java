...
public class FlyingPathFinder extends PathFinder {
    ...
    // createEntityPathTo
    @Override
    public Path findPath(@Nonnull IBlockAccess blockaccess, @Nonnull EntityLiving entityFrom, @Nonnull Entity entityTo,
                         float dist) {
        return createEntityPathTo(blockaccess, entityFrom, entityTo.posX, entityTo.getEntityBoundingBox().minY,
                entityTo.posZ, dist);
    }

    // createEntityPathTo
    @Override
    public Path findPath(@Nonnull IBlockAccess blockaccess, @Nonnull EntityLiving entityIn, @Nonnull BlockPos targetPos,
                         float dist) {
        return createEntityPathTo(blockaccess, entityIn, targetPos.getX() + 0.5F, targetPos.getY() + 0.5F,
                targetPos.getZ() + 0.5F, dist);
    }

    private Path createEntityPathTo(@Nonnull IBlockAccess blockaccess, @Nonnull Entity ent, double x, double y,
        ...
        PathPoint climbPoint = new PathPoint(rnd(startPoint.x + offset.x), rnd(startPoint.y + offset.y),
                rnd(startPoint.z + offset.z));
    ...
    private PathPoint[] addToPath(@Nonnull Entity entityIn, @Nonnull PathPoint pathpointStart,
                                  @Nonnull PathPoint pathpointEnd, float maxDistance) {
        ...
        while (!path.isPathEmpty()) {
            ...
            // int numPathOptions = nodeProcessor.findPathOptions(pathOptions, entityIn, dequeued, pathpointEnd,
            // maxDistance);

            for (int j = 0; j < numPathOptions; ++j) {
                PathPoint cadidatePoint = pathOptions[j];
                if (cadidatePoint != null) {
                    float newTotalDistance = PPUtil.getTotalPathDistance(dequeued) +
                            dequeued.distanceToSquared(cadidatePoint);
                    if (newTotalDistance < maxDistance * 2.0F && (!cadidatePoint.isAssigned() ||
                            newTotalDistance < PPUtil.getTotalPathDistance(cadidatePoint))) {
                        ...
                        if (cadidatePoint.isAssigned()) {
                            // path.changeDistance(cadidatePoint, cadidatePoint.totalPathDistance +
                            // cadidatePoint.distanceToNext);
                            path.changeDistance(cadidatePoint, PPUtil.getTotalPathDistance(cadidatePoint) +
                                    PPUtil.getDistanceToNext(cadidatePoint));
                        } else {
                            PPUtil.setDistanceToTarget(cadidatePoint, PPUtil.getTotalPathDistance(cadidatePoint) +
                                    PPUtil.getDistanceToNext(cadidatePoint));
    ...
    private Path createDefault(@Nonnull IBlockAccess blockaccess, @Nonnull EntityLiving entityIn, float distance,
    ...
    private static PathPoint[] createEntityPath(PathPoint start, PathPoint end) {
        ...
        for (PathPoint pathpoint = end; pathpoint != null &&
                PPUtil.getPrevious(pathpoint) != null; pathpoint = PPUtil.getPrevious(pathpoint)) {
            ++i;
        }
        ...
        --i;
        for (apathpoint[i] = end; pathpoint1 != null &&
                PPUtil.getPrevious(pathpoint1) != null; apathpoint[i] = pathpoint1) {
