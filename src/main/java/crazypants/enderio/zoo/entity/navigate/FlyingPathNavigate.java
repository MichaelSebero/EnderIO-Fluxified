...
public class FlyingPathNavigate extends PathNavigateGround {
    ...
    public boolean tryFlyToXYZ(double x, double y, double z, double speedIn) {
        Path pathentity = getPathToPos(
                new BlockPos((double) MathHelper.floor(x), (double) ((int) y), (double) MathHelper.floor(z)));
    ...
    protected void pathFollow() {
        ...
        // there
        int i = 6;
        for (int j = Math.min(currentPath2.getCurrentPathIndex() + i, currentPath2.getCurrentPathLength() - 1); j >
                currentPath2.getCurrentPathIndex(); --j) {
    ...
    @Override
    protected boolean isDirectPathBetweenPoints(@Nonnull Vec3d startPos, @Nonnull Vec3d endPos, int sizeX, int sizeY,
                                                int sizeZ) {
