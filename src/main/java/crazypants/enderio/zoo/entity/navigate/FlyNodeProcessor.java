...
public class FlyNodeProcessor extends WalkNodeProcessor {
    ...
    public @Nonnull PathPoint getStart() {
        ...
        return openPoint(MathHelper.floor(entityIn.getEntityBoundingBox().minX),
                MathHelper.floor(entityIn.getEntityBoundingBox().minY + 0.5D),
    ...
    public @Nonnull PathPoint getPathPointToCoords(double x, double y, double z) {
        EntityLiving entityIn = entity;
        return openPoint(MathHelper.floor(x - entityIn.width / 2.0F), MathHelper.floor(y),
                MathHelper.floor(z - entityIn.width / 2.0F));
    }

    @Override
    public int findPathOptions(@Nonnull PathPoint[] pathOptions, @Nonnull PathPoint currentPoint,
                               @Nonnull PathPoint targetPoint, float maxDistance) {
        EntityLiving entityIn = entity;
        int i = 0;
        for (EnumFacing enumfacing : EnumFacing.values()) {
            PathPoint pathpoint = getSafePoint(entityIn, currentPoint.x + enumfacing.getXOffset(),
