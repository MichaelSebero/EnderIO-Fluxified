...
public class EntityEpicSquid extends EntitySquid implements IEnderZooEntity {
    ...
    public static void onPreInit(EnderIOLifecycleEvent.PreInit event) {
        RenderingRegistry.registerEntityRenderingHandler(EntityEpicSquid.class,
                renderManager -> new RenderEpicSquid(renderManager));
    }
    ...
    public boolean handleWaterMovement() {
        ...
        if (!inWater) {
            if (world.handleMaterialAcceleration(
                    this.getEntityBoundingBox().grow(0.0D, -0.4000000059604645D, 0.0D).shrink(0.001D), Material.LAVA,
                    this)) {
                fallDistance = 0.0F;
                inWater = true;
                extinguish();
