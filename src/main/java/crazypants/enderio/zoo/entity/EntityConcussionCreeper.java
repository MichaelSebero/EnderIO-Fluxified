...
public class EntityConcussionCreeper extends EntityCreeper implements IEnderZooEntity.Aggressive {
    ...
    public static void onPreInit(EnderIOLifecycleEvent.PreInit event) {
        RenderingRegistry.registerEntityRenderingHandler(EntityConcussionCreeper.class,
                RenderConcussionCreeper.FACTORY);
    }
    ...
    public void onUpdate() {
        if (isEntityAlive()) {
            ...
            if (timeSinceIgnited >= fuseTime - 1) {
                ...
                AxisAlignedBB bb = new AxisAlignedBB(posX - range, posY - range, posZ - range, posX + range,
                        posY + range, posZ + range);
                List<EntityLivingBase> ents = world.getEntitiesWithinAABB(EntityLivingBase.class, bb);
                for (EntityLivingBase ent : ents) {
                    if (!ent.isDead) {
                        if (!world.isRemote) {
                            RandomTeleportUtil.teleportEntity(ent.world, ent, false, true,
                                    ZooConfig.teleportRange.get());
                        }
                        if (ent instanceof EntityPlayer) {
                            EnderIOZoo.proxy.setInstantConfusionOnPlayer((EntityPlayer) ent,
                                    ZooConfig.confusionDuration.get());
                        }
                    }
                }
            }
