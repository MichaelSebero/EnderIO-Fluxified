...
public final class MobSpawns {
    ...
    public void addSpawn(ISpawnEntry entry) {
        ...
        Class<? extends EntityLiving> clz = (Class<? extends EntityLiving>) EntityList
                .getClass(new ResourceLocation(mobname));
        if (clz == null) {
        ...
        for (IBiomeFilter filter : entry.getFilters()) {
            Biome[] biomes = filter.getMatchedBiomes();
            printBiomeNames(biomes);
            EntityRegistry.addSpawn(clz, entry.getRate(), entry.getMinGroupSize(), entry.getMaxGroupSize(),
                    entry.getCreatureType(), biomes);
        }
    }
