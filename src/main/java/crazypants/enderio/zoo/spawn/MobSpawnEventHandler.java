...
public class MobSpawnEventHandler {
    ...
    public static void onBlockHarvest(HarvestDropsEvent event) {
        if ((!ZooConfig.direSlimeEnabled.get() && !ZooConfig.direSlimeEnabledHand.get()) || event.isCanceled() ||
                event.getWorld() == null || event.getWorld().isRemote || event.getHarvester() == null ||
                event.getHarvester().capabilities.isCreativeMode ||
                !(event.getState().getBlock() instanceof BlockDirt ||
                        event.getState().getBlock() instanceof BlockGrass)) {
            return;
        }
        ...
        if (!isToolEffective(event.getState(), event.getHarvester().getHeldItemMainhand()) &&
            ...
            direSlime.setPosition(event.getPos().getX() + 0.5, event.getPos().getY() + 0.0,
        ...
        for (String type : stack.getItem().getToolClasses(stack)) {
            if (type != null) {
                if (state.getBlock().isToolEffective(type, state) || "shovel".equals(type)) {
                    // the "shovel" check is needed for modded blocks that extend dirt/grass but refuse to acknowledge
                    // any tool as effective, e.g. TiC
                    return true;
                }
            }
        }
        ...
        for (ISpawnEntry ent : MobSpawns.instance.getEntries()) {
            if (resourceLocation.toString().equals(ent.getMobName())) {
                if (!ent.canSpawnInDimension(evt.getWorld())) {
                    // Log.debug(resourceLocation, " may not spawn in dimension ",
                    // evt.getWorld().provider.getDimensionType().getName());
                    evt.setResult(Result.DENY);
                    return;
                } else {
                    // Log.debug(resourceLocation, " is allowed to spawn in dimension ",
                    // evt.getWorld().provider.getDimensionType().getName());
                }
            }
        }
