...
public class SpawnConfigParser extends DefaultHandler {
    ...
    private static final BiomeDictionary.Type[] BASE_LAND_TYPES_ARR = new BiomeDictionary.Type[] {
            BiomeDictionary.Type.MESA, BiomeDictionary.Type.FOREST,
            BiomeDictionary.Type.PLAINS, BiomeDictionary.Type.MOUNTAIN, BiomeDictionary.Type.HILLS,
            BiomeDictionary.Type.SWAMP, BiomeDictionary.Type.SANDY,
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
        ...
        if (ELEMENT_ROOT.equals(localName)) {
            ...
            if (currentEntry == null && !invalidEntryElement) {
                Log.warn(ELEMENT_DIM_EXCLUDE + " found outside an " + ELEMENT_ENTRY + " and/or " + ELEMENT_FILTER +
                        " element. It will be ignored");
            ...
            if (currentEntry == null && !invalidEntryElement) {
                Log.warn(ELEMENT_DIM_INCLUDE + " found outside an " + ELEMENT_ENTRY + " and/or " + ELEMENT_FILTER +
                        " element. It will be ignored");
            } else if (currentEntry != null) {
    ...
    private void parseEntry(Attributes attributes) {
        ...
        if (creatureType != null) {
            try {
                ...
                Log.warn(
                        "Invalid value specified for " + ATT_CREATURE_TYPE + " in entry " + id +
                                " using default value " + currentEntry.getCreatureType() + " error: " + e);
            }
        }
        ...
        if (minGrp != -1) {
            if (minGrp < 0) {
                Log.warn("Value less than 0 found for " + ATT_MIN_GRP + " in entry " + id + " using default value " +
        ...
        if (maxGrp != -1) {
            if (maxGrp < currentEntry.getMinGroupSize()) {
                Log.warn("Value for " + ATT_MAX_GRP + " in entry " + id + " less than " + ATT_MIN_GRP +
                        " using default " + currentEntry.getMaxGroupSize());
            } else {
    ...
    private void parseFilter(Attributes attributes) {
        ...
        if (isEmptyString(typeStr)) {
            Log.warn("Attribue " + ATT_TYPE + " not specified for element " + ELEMENT_FILTER + " defaulting to '" +
                    FILTER_TYPE_ANY + "' filter");
    ...
    private void parseBiomeType(Attributes attributes) {
        ...
        if (nameEmpty && typeEmpty) {
            Log.warn("Attribute " + ATT_NAME + " or " + ATT_TYPE + " not specified in element " + ELEMENT_BIOME +
                    " in entry " + currentEntry.getId());
            return;
        }
        if (!nameEmpty && !typeEmpty) {
            Log.warn("Attribute " + ATT_NAME + " and " + ATT_TYPE + " both specified in element " + ELEMENT_BIOME +
                    " in entry " + currentEntry.getId() + ". It will be ignored");
            return;
        }
        ...
        if (!typeEmpty) {
            ...
            if (BASE_LAND_TYPES.equals(biomeType)) {
                ...
                try {
                    ...
                    Log.warn("Attribute " + ATT_TYPE + " in element " + ELEMENT_BIOME + " with value " + biomeType +
    ...
    public static int getIntValue(String qName, Attributes attributes, int def) {
        try {
            ...
            Log.warn("Could not parse a valid int for attribute " + qName + " with value " +
    ...
    public static float getFloatValue(String qName, Attributes attributes, float def) {
        try {
            ...
            Log.warn("Could not parse a valid float for attribute " + qName + " with value " +
                    getStringValue(qName, attributes, null));
            return def;
        }
    }
