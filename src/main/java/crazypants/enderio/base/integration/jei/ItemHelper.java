  ...
  protected static final class Walker implements Callback<ItemStack> {
    ...
    public void apply(@Nonnull ItemStack stack) {
      if (Prep.isInvalid(stack)) {
        Log.error("The item " + item + " (" + item.getTranslationKey() + ") produces empty itemstacks in getSubItems(). This is a major bug in the mod '"
            + PaintUtil.block2Modname(item) + "'.");
        return;
      }
      if (stack.getItem() == Items.AIR) {
        Log.error("The item " + item + " (" + item.getTranslationKey()
