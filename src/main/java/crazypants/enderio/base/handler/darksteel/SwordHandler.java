...
public class SwordHandler {
    ...
    public static void onEntityDrop(LivingDropsEvent evt) {
        ...
        dropSkull(evt, player,
                ((int) skullDropChance) + (killedMob.world.rand.nextDouble() < (skullDropChance % 1) ? 1 : 0));
        ...
        if (isEquipped(player)) {
            ...
            if (isEnderman) {
                ...
                for (EntityItem stack : evt.getDrops()) {
                    if (stack.getItem().getItem() == dropItem.getItem() &&
                            stack.getItem().getItemDamage() == dropItem.getItemDamage()) {
                ...
                if (toDrop > 0) {
                    ...
                    evt.getDrops().add(Util.createDrop(player.world, dropItem, killedMob.posX, killedMob.posY,
                            killedMob.posZ, false));
                }
    ...
    private static double getSkullDropChance(@Nonnull EntityPlayer player, LivingDropsEvent evt) {
        ...
        final float fakeplayer = iee(isWitherSkeleton, isEnderman, HEAD_FAKEPLAYER_CHANCE,
                HEAD_FAKEPLAYER_CHANCE_WITHER, HEAD_FAKEPLAYER_CHANCE_ENDERMAN)
                        .getFloat(player instanceof FakePlayer ? 0 : 1);

        if (isEquipped(player)) {
            looting = iee(isWitherSkeleton, isEnderman, HEAD_CHANCE, HEAD_CHANCE_WITHER, HEAD_CHANCE_ENDERMAN)
                    .getFloat(evt.getLootingLevel());
            empowered = iee(isWitherSkeleton, isEnderman, HEAD_EMPOWERED_CHANCE, HEAD_EMPOWERED_CHANCE_WITHER,
                    HEAD_EMPOWERED_CHANCE_ENDERMAN)
                            .getFloat(isEquippedAndPowered(player, DarkSteelConfig.darkSteelSwordPowerUsePerHit) ?
                                    EnergyUpgradeManager.getPowerUpgradeLevel(equipped) + 1 : 0);
            tier = HEAD_TIER_CHANCE.getFloat(((ItemDarkSteelSword) equipped.getItem()).getEquipmentData().getTier());
        } else {
            looting = iee(isWitherSkeleton, isEnderman, HEAD_VANILLA_CHANCE, HEAD_VANILLA_CHANCE_WITHER,
                    HEAD_VANILLA_CHANCE_ENDERMAN)
                            .getFloat(evt.getLootingLevel());
            empowered = 0;
            tier = 1;
        }
    ...
    private static void dropSkull(LivingDropsEvent evt, EntityPlayer player, int amount) {
        if (amount > 0) {
            ...
            if (skull != null && !skull.isEmpty() && !containsDrop(evt, skull)) {
                ...
                evt.getDrops().add(Util.createEntityItem(player.world, skull, evt.getEntityLiving().posX,
                        evt.getEntityLiving().posY, evt.getEntityLiving().posZ));
            }
        }
    }
    ...
    private static boolean containsDrop(LivingDropsEvent evt, @Nonnull ItemStack skull) {
        for (EntityItem ei : evt.getDrops()) {
            if (ei != null && ei.getItem().getItem() == skull.getItem() &&
                    ei.getItem().getItemDamage() == skull.getItemDamage()) {
                return true;
            }
        }
        return false;
    ...
    private static @Nullable ItemStack getSkullForEntity(EntityLivingBase entityLiving) {
        ...
        if (entityLiving instanceof EntitySkeleton) {
            ...
            try {
                ...
                return (ItemStack) ReflectionHelper.findMethod(EntityZombie.class, "getSkullDrop", "func_190732_dj")
                        .invoke(entityLiving);
