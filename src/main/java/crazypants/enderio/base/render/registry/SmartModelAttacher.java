...
public class SmartModelAttacher {
  ...
  public static void bakeModels(@Nonnull ModelBakeEvent event) {
    for (RegistrationHolder holder : blocks) {
      ...
      if (holder_property != null && block.getDefaultState().getPropertyKeys().contains(holder_property) && holder_defaultsValue != null) {
        ...
        ModelResourceLocation itemMrl = new ModelResourceLocation(defaultMrl.getNamespace() + ":" + defaultMrl.getPath() + "#inventory");
  ...
  private static String debugOutput(IRegistry<ModelResourceLocation, IBakedModel> modelRegistry, ResourceLocation defaultMrl, IBakedModel missingModel) {
    String prefix = defaultMrl.getNamespace() + ":" + defaultMrl.getPath();
