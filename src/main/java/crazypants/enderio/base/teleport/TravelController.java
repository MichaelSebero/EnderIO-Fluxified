...
public class TravelController {
    ...
    public static boolean activateTravelAccessable(@Nonnull ItemStack equipped, @Nonnull EnumHand hand,
                                                   @Nonnull World world, @Nonnull EntityPlayer player,
    ...
    public static boolean doBlink(@Nonnull ItemStack equipped, @Nonnull EnumHand hand, @Nonnull EntityPlayer player) {
        ...
        if (p == null) {
            ...
            List<RayTraceResult> res = Util.raytraceAll(player.world, eye3, end,
                    !TeleportConfig.enableBlinkNonSolidBlocks.get());
                ...
                if (pos != null) {
                    ...
                    if (isBlackListedBlock(player, pos, hitBlock)) {
                        ...
                        maxDistance = Math.min(maxDistance,
                                VecmathUtil.distance(eye,
                                        new Vector3d(bp.getX() + 0.5, bp.getY() + 0.5, bp.getZ() + 0.5)) - 1.5 -
                                        lookComp);
                    }
                }
            }
            ...
            double teleDistance = VecmathUtil.distance(eye,
                    new Vector3d(bp.getX() + 0.5, bp.getY() + 0.5, bp.getZ() + 0.5)) + sampleDistance;
            ...
            sampleDistance = -0.5;
            teleDistance = VecmathUtil.distance(eye, new Vector3d(bp.getX() + 0.5, bp.getY() + 0.5, bp.getZ() + 0.5)) +
                    sampleDistance;
    ...
    private static boolean isBlackListedBlock(@Nonnull EntityPlayer player, @Nonnull RayTraceResult pos,
        ...
        return TeleportConfig.blockBlacklist.get().contains(hitBlock.getBlock()) ||
                (hitBlock.getBlockHardness(player.world, pos.getBlockPos()) < 0 &&
                        !TeleportConfig.enableBlinkUnbreakableBlocks.get());
    }
    ...
    public static boolean isTravelItemActiveForRendering(@Nonnull EntityPlayer ep) {
        return isTravelItemActive(ep, ep.getHeldItemMainhand()) ||
                (TeleportConfig.enableOffHandTravel.get() && isTravelItemActive(ep, ep.getHeldItemOffhand()));
    }
    ...
    private static boolean isTravelItemActive(@Nonnull EntityPlayer ep, @Nonnull ItemStack equipped) {
        return equipped.getItem() instanceof IItemOfTravel &&
                ((IItemOfTravel) equipped.getItem()).isActive(ep, equipped);
    }

    private static boolean travelToSelectedTarget(@Nonnull EntityPlayer player, @Nonnull ItemStack equipped,
    ...
    private static boolean travelToLocation(@Nonnull EntityPlayer player, @Nonnull ItemStack equipped,
        ...
        if (doClientTeleport(player, hand, coord, source, requiredPower, conserveMomentum) &&
                PersonalConfig.machineParticlesEnabled.get()) {
            for (int i = 0; i < 6; ++i) {
                player.world.spawnParticle(EnumParticleTypes.PORTAL, player.posX + (rand.nextDouble() - 0.5D),
    ...
    private static int getRequiredPower(@Nonnull EntityPlayer player, @Nonnull ItemStack equipped,
                                        @Nonnull TravelSource source, @Nonnull BlockPos coord) {
    ...
    private static boolean isValidTarget(@Nonnull EntityPlayer player, @Nonnull BlockPos bc,
                                         @Nonnull TravelSource source) {
        ...
        return canTeleportTo(player, source, baseLoc, w) &&
                canTeleportTo(player, source, baseLoc.offset(EnumFacing.UP), w);
    }

    private static boolean canTeleportTo(@Nonnull EntityPlayer player, @Nonnull TravelSource source,
                                         @Nonnull BlockPos bc, @Nonnull World w) {
        if (bc.getY() < 1) {
            return false;
        }
    ...
    private static boolean canBlinkTo(@Nonnull BlockPos bc, @Nonnull World w, @Nonnull Vec3d start,
                                      @Nonnull Vec3d target) {
    ...
    private static boolean isClear(@Nonnull World w, @Nonnull IBlockState bs, @Nonnull Block block,
                                   @Nonnull BlockPos bp) {
    ...
    public static boolean doClientTeleport(@Nonnull Entity entity, @Nonnull EnumHand hand, @Nonnull BlockPos bc,
                                           @Nonnull TravelSource source, int powerUse,
                                           boolean conserveMomentum) {
    ...
    private static Pair<BlockPos, ITravelAccessable> getActiveTravelBlock(@Nonnull EntityPlayerSP player) {
        ...
        if (NullHelper.untrust(world) == null) {
            // Log.warn("(in TickEvent.ClientTickEvent) net.minecraft.client.Minecraft.world is marked @Nonnull but it
            // is null.");
            return null;
        }
