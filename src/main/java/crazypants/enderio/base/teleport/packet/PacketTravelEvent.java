...
public class PacketTravelEvent implements IMessage {
    ...
    public static class Handler implements IMessageHandler<PacketTravelEvent, IMessage> {
        ...
        public IMessage onMessage(PacketTravelEvent message, MessageContext ctx) {
            ...
            doServerTeleport(toTp, BlockPos.fromLong(message.pos), message.powerUse, message.conserveMotion, source,
                    hand);

            return null;
        }

        private boolean doServerTeleport(@Nonnull Entity toTp, @Nonnull BlockPos pos, int powerUse,
            ...
            if (player != null) {
                if (conserveMotion) {
                    ...
                    SPacketEntityVelocity p = new SPacketEntityVelocity(toTp.getEntityId(), velocityVex.x,
                            velocityVex.y, velocityVex.z);
