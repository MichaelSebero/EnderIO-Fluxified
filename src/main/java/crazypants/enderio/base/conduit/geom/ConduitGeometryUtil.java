...
public final class ConduitGeometryUtil {
  ...
  private @Nonnull BoundingBox createConnectorComponent(@Nonnull EnumFacing dir, float cornerMin, float cornerMax, float depthMin, float depthMax) {
    float minX = (1 - Math.abs(dir.getXOffset())) * cornerMin + dir.getXOffset() * depthMin;
    float minY = (1 - Math.abs(dir.getYOffset())) * cornerMin + dir.getYOffset() * depthMin;
    float minZ = (1 - Math.abs(dir.getZOffset())) * cornerMin + dir.getZOffset() * depthMin;

    float maxX = (1 - Math.abs(dir.getXOffset())) * cornerMax + (dir.getXOffset() * depthMax);
    float maxY = (1 - Math.abs(dir.getYOffset())) * cornerMax + (dir.getYOffset() * depthMax);
    float maxZ = (1 - Math.abs(dir.getZOffset())) * cornerMax + (dir.getZOffset() * depthMax);
