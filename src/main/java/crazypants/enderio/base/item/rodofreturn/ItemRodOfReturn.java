...
public class ItemRodOfReturn extends AbstractPoweredItem implements IAdvancedTooltipProvider, IOverlayRenderAware {
    ...
    private static final int RF_MAX_INPUT = (int) Math
            .ceil(ItemConfig.rodOfReturnPowerStorage.get() / (double) ItemConfig.rodOfReturnMinTicksToRecharge.get());
    ...
    @Override
    public @Nonnull EnumActionResult onItemUseFirst(@Nonnull EntityPlayer player, @Nonnull World world,
        ...
        if (te instanceof ITelePad) {
            ...
            if (tp != null) {
                ...
                player.sendMessage(
                        Lang.RETURN_ROD_SYNC_TELEPAD.toChat(BlockCoord.chatString(pos, TextFormatting.WHITE)));
    ...
    @Override
    public @Nonnull ItemStack onItemUseFinish(@Nonnull ItemStack stack, @Nonnull World worldIn,
        ...
        if (hasPower && hasFluid) {
            ...
            TeleportUtil.doTeleport(entityLiving, target.getLocation(), target.getDimension(), false,
                    TravelSource.TELEPAD);
    ...
    public void addInformation(@Nonnull ItemStack stack, @Nullable World worldIn, @Nonnull List<String> tooltip,
        ...
        if (usesFluid()) {
            tooltip.add(Lang.RETURN_ROD_FLUID
                    .get(LangFluid.MB(FLUIDAMOUNT.getInt(stack, 0), ItemConfig.rodOfReturnFluidStorage.get())));
        }
        tooltip.add(Lang.RETURN_ROD_POWER
                .get(LangPower.RF(getEnergyStored(stack), ItemConfig.rodOfReturnPowerStorage.get())));
    }
    ...
    private void onUsingClient(ItemStack stack, EntityLivingBase player, int timeLeft) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (fx != null) {
                // if(rand.nextInt(8) == 0) {
                // fx.setRBGColorF((rand.nextFloat() * 0.1f), 0.6f + (rand.nextFloat() * 0.15f), 0.75f +
                // (rand.nextFloat() * 0.2f));
                // }
    ...
    public void addBasicEntries(@Nonnull ItemStack itemstack, @Nullable EntityPlayer entityplayer,
                                @Nonnull List<String> list, boolean flag) {
        SpecialTooltipHandler.addBasicTooltipFromResources(list, getTranslationKey());
    }
    ...
    public void addDetailedEntries(@Nonnull ItemStack itemstack, @Nullable EntityPlayer entityplayer,
        ...
        SpecialTooltipHandler.addDetailedTooltipFromResources(entries, getTranslationKey());
        String fluidString = usesFluid() ? ItemConfig.rodOfReturnFluidType.get()
                .getLocalizedName(new FluidStack(ItemConfig.rodOfReturnFluidType.get(), 1000)) : "---";
    ...
    public int fill(@Nonnull ItemStack container, @Nonnull FluidStack resource, boolean doFill) {
        if (!(container.getItem() == this) || resource.amount <= 0 || resource.getFluid() == null ||
                resource.getFluid() != ItemConfig.rodOfReturnFluidType.get() || !usesFluid()) {
            return 0;
        }
        int amount = FLUIDAMOUNT.getInt(container, 0);
    ...
    private final class FluidCapabilityProvider implements IFluidHandlerItem, ICapabilityProvider {
        ...
        public int fill(FluidStack resource, boolean doFill) {
            return ItemRodOfReturn.this.fill(container,
                    NullHelper.notnull(resource, "Cannot use null as a fluid stack"), doFill);
        }
