...
public class CapBankClientNetwork implements ICapBankNetwork {
  ...
  private @Nonnull IOInfo computeIODisplayInfo(int xOrg, int yOrg, int zOrg, @Nonnull EnumFacing dir) {
    ...
    while (isIOType(xOrg + left.getXOffset(), yOrg, zOrg + left.getZOffset(), dir, type)) {
      xOrg += left.getXOffset();
      zOrg += left.getZOffset();
      hOff++;
    }
    ...
    if (isIOType(xOrg + left.getXOffset(), yOrg, zOrg + left.getZOffset(), dir, type)) {
      // not a rectangle
      return IOInfo.SINGLE;
    }
    ...
    int yTmp = yOrg;
    int zTmp = zOrg;
    while (isIOType(xTmp + right.getXOffset(), yTmp, zTmp + right.getZOffset(), dir, type)) {
      if (isIOType(xTmp + right.getXOffset(), yTmp + 1, zTmp + right.getZOffset(), dir, type)) {
        // not a rectangle
        return IOInfo.SINGLE;
      }
      xTmp += right.getXOffset();
      zTmp += right.getZOffset();
      width++;
    }
    ...
    while (isIOType(xOrg, yTmp - 1, zOrg, dir, type)) {
      ...
      if (isIOType(xTmp + left.getXOffset(), yTmp, zTmp + left.getZOffset(), dir, type)) {
        // not a rectangle
        return IOInfo.SINGLE;
      }
      ...
      if (isIOType(xTmp + right.getXOffset(), yTmp, zTmp + right.getZOffset(), dir, type)) {
        // not a rectangle
        return IOInfo.SINGLE;
      }
