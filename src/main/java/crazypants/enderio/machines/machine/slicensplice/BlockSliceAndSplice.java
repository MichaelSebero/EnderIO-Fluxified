  ...
  public void randomDisplayTick(@Nonnull IBlockState bs, @Nonnull World world, @Nonnull BlockPos pos, @Nonnull Random rand) {
    ...
    if (PersonalConfig.machineParticlesEnabled.get() && te != null && isActive(world, pos)) {
      ...
      for (int i = 0; i < count; i++) {
        double px = x + 0.5 + front.getXOffset() * 0.5;
        double pz = z + 0.5 + front.getZOffset() * 0.5;
        double v = 0.1;
        double vx = 0;
        double vz = 0;
        ...
        if (rand.nextFloat() > 0.3f) {
          ...
          Minecraft.getMinecraft().effectRenderer
              .addEffect(new ParticleBloodDrip(world, px + front.getXOffset() * 0.1, y + 0.325, pz + front.getZOffset() * 0.1, front));
        }
      }
    }
  }
