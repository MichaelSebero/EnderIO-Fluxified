...
public class TileTelePad extends TileTravelAnchor
    ...
    public TileTelePad() {
        ...
        getInventory().getSlot(EnergyLogic.CAPSLOT).set(new ItemStack(ModObject.itemBasicCapacitor.getItemNN(), 1,
                DefaultCapacitorData.ENDER_CAPACITOR.ordinal()));
        ...
        // Note: Other capabilities are not relayed to the master. Other mods that attach caps to foreign blocks
        // probably don't
        ...
        if (active()) {
            if (activeSound == null) {
                ...
                FMLClientHandler.instance().getClient().getSoundHandler()
                        .playSound(activeSound = new MachineSound(ACTIVE_RES, p.getX(), p.getY(), p.getZ(), 0.3f, 1));
            }
            updateQueuedEntities();
    ...
    private int calculateTeleportPower() {
        if (world.provider.getDimension() == target.getDimension()) {
            int distance = (int) Math.ceil(pos.getDistance(target.getLocation().getX(), target.getLocation().getY(),
        ...
        if (TelePadConfig.telepadFluidUse.get() > 0) {
            if (tank.getFluidAmount() < TelePadConfig.telepadFluidUse.get()) {
                ...
                if (entity instanceof EntityPlayer) {
                    ((EntityPlayer) entity).sendMessage(
                            Lang.GUI_TELEPAD_NOFLUID.toChatServer(new FluidStack(fluidType, 1).getLocalizedName()));
                }
                wasBlocked = true;
                return true;
            }
    ...
    private boolean clientTeleport(@Nonnull Entity entity) {
        return TeleportUtil.checkClientTeleport(entity, target.getLocation(), target.getDimension(),
                TravelSource.TELEPAD);
    }

    private boolean serverTeleport(@Nonnull Entity entity) {
        dequeueTeleport(entity, true);
        return TeleportUtil.serverTeleport(entity, target.getLocation(), target.getDimension(), false,
                TravelSource.TELEPAD);
    }
     ...
     * {@link #getCapability(Capability, EnumFacing)}. If that doesn't yield a result, returns DENY to stop the current
     * tile's superclasses fro providing a value.
     * <p>
